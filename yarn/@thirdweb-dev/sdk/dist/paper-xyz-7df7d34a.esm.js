import { z } from 'zod';
import { Q as QuantitySchema, N as NFTInputOrUriSchema, C as CommonNFTOutput, a as CommonNFTInput, _ as _defineProperty, g as getRpcUrl, D as DEFAULT_QUERY_ALL_COUNT } from './QueryParams-7ff2dcd2.esm.js';
...
class ShardedMerkleTree {
  ...
  static hashEntry(entry, tokenDecimals) {
    return hashLeafNode(entry.address, utils.parseUnits(entry.maxClaimable, tokenDecimals));
  }

  static async buildAndUpload(snapshotInput, tokenDecimals, storage) {
    let shardNybbles = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : SHARD_NYBBLES;
    ...
    const roots = Object.fromEntries(Object.entries(shards).map(_ref => {
      let [shard, entries] = _ref;
      return [shard, new MerkleTree(entries.map(entry => ShardedMerkleTree.hashEntry(entry, tokenDecimals)), utils.keccak256, {
        sort: true
      }).getHexRoot()];
  ...
  async getProof(address) {
    ...
    if (shard === undefined) {
      try {
        ...
        this.trees[shardId] = new MerkleTree(shard.entries.map(entry => ShardedMerkleTree.hashEntry(entry, this.tokenDecimals)), utils.keccak256, {
          sort: true
        });
      } catch (e) {
  ...
  async getAllEntries() {
    try {
      const entries = await this.storage.downloadJSON(this.originalEntriesUri);
      return entries;
    } catch (e) {
  ...
  computeAllShardIds(shardNybbles) {
...
async function createSnapshot(snapshotInput, tokenDecimals, storage) {
  ...
  const tree = await ShardedMerkleTree.buildAndUpload(input, tokenDecimals, storage);
  return {
...
async function prepareClaim(quantity, activeClaimCondition, merkleMetadataFetcher, tokenDecimals, contractWrapper, storage, checkERC20Allowance) {
  ...
  try {
    if (!activeClaimCondition.merkleRootHash.toString().startsWith(constants.AddressZero)) {
      const claim = await fetchSnapshotEntryForAddress(addressToClaim, activeClaimCondition.merkleRootHash.toString(), await merkleMetadataFetcher(), storage);
...
async function fetchSnapshotEntryForAddress(address, merkleRoot, merkleMetadata, storage) {
  if (!merkleMetadata) {
    return null;
  }
...
async function getClaimerProofs(addressToClaim, merkleRoot, tokenDecimals, merkleMetadata, storage) {
  const claim = await fetchSnapshotEntryForAddress(addressToClaim, merkleRoot, merkleMetadata, storage);
...
const EditionDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract, _network] = await Promise.all([EditionDropInitializer.getAbi(), import('./edition-drop-a3aff4bf.esm.js'), provider.getNetwork()]);
...
const EditionInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([EditionInitializer.getAbi(), import('./edition-0b5e2603.esm.js'), provider.getNetwork()]);
...
const MarketplaceInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceInitializer.getAbi(), import('./marketplace-f49929a7.esm.js'), provider.getNetwork()]);
...
const MultiwrapInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MultiwrapInitializer.getAbi(), import('./multiwrap-7fcaf2f0.esm.js'), provider.getNetwork()]);
...
const NFTCollectionInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTCollectionInitializer.getAbi(), import('./nft-collection-62b2ee54.esm.js'), provider.getNetwork()]);
...
const NFTDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTDropInitializer.getAbi(), import('./nft-drop-2b00962e.esm.js'), provider.getNetwork()]);
...
const PackInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([PackInitializer.getAbi(), import('./pack-7950d576.esm.js'), provider.getNetwork()]);
...
const SignatureDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SignatureDropInitializer.getAbi(), import('./signature-drop-ae8ddfd4.esm.js'), provider.getNetwork()]);
...
const SplitInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SplitInitializer.getAbi(), import('./split-59a0c211.esm.js'), provider.getNetwork()]);
...
const TokenDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenDropInitializer.getAbi(), import('./token-drop-1e8b1e07.esm.js'), provider.getNetwork()]);
...
const TokenInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenInitializer.getAbi(), import('./token-e6f5f523.esm.js'), provider.getNetwork()]);
...
const VoteInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([VoteInitializer.getAbi(), import('./vote-88b47b98.esm.js'), provider.getNetwork()]);
...
class ContractFactory extends ContractWrapper {
  ...
  async deploy(contractType, contractMetadata) {
    ...
    try {
      ...
      console.log(e); // if the error is caused by user cancelling the transaction, just re-throw it
...
export { FullPublishMetadataSchemaInput as $, MintRequest721withQuantity as A, BaseSignaturePayloadInput as B, CommonContractSchema as C, MerkleSchema as D, EditionMetadataOutputSchema as E, SnapshotEntryInput as F, SnapshotInputSchema as G, SnapshotSchema as H, SnapshotInfoSchema as I, BYOCContractMetadataSchema as J, CustomContractInput as K, CustomContractOutput as L, MintRequest20 as M, CustomContractDeploy as N, OptionalPropertiesInput as O, PartialClaimConditionInputSchema as P, CustomContractSchema as Q, AbiTypeSchema as R, SUPPORTED_CHAIN_IDS as S, TokenMintInputSchema as T, AbiObjectSchema as U, AbiSchema as V, PreDeployMetadata as W, ChainIdToAddressSchema as X, FactoryDeploymentSchema as Y, ExtraPublishMetadataSchemaInput as Z, ExtraPublishMetadataSchemaOutput as _, ClaimConditionInputSchema as a, VoteType as a$, FullPublishMetadataSchemaOutput as a0, ProfileSchemaInput as a1, ProfileSchemaOutput as a2, PublishedContractSchema as a3, ContractInfoSchema as a4, CompilerMetadataFetchedSchema as a5, PreDeployMetadataFetchedSchema as a6, LoginOptionsSchema as a7, LoginPayloadDataSchema as a8, LoginPayloadSchema as a9, Erc721Enumerable as aA, Erc721Mintable as aB, Erc721BatchMintable as aC, Erc721Burnable as aD, StandardErc721 as aE, Erc1155 as aF, Erc1155Enumerable as aG, Erc1155Mintable as aH, Erc1155BatchMintable as aI, Erc1155SignatureMintable as aJ, Erc1155Burnable as aK, Erc1155LazyMintable as aL, StandardErc1155 as aM, MarketplaceDirect as aN, MarketplaceAuction as aO, ContractDeployer as aP, GasCostEstimator as aQ, ContractEvents as aR, ContractInterceptor as aS, ContractPlatformFee as aT, ContractPublishedMetadata as aU, ContractOwner as aV, TransactionTask as aW, UserWallet as aX, ThirdwebSDK as aY, ListingType as aZ, ProposalState as a_, VerifyOptionsSchema as aa, AuthenticationOptionsSchema as ab, AuthenticationPayloadDataSchema as ac, AuthenticationPayloadSchema as ad, WalletAuthenticator as ae, ContractEncoder as af, ContractMetadata as ag, ContractRoles as ah, ContractRoyalty as ai, ContractPrimarySale as aj, DelayedReveal as ak, DropClaimConditions as al, DropErc1155ClaimConditions as am, DropErc1155History as an, Erc20 as ao, Erc20Mintable as ap, Erc20BatchMintable as aq, TokenERC20History as ar, Erc20SignatureMintable as as, Erc20Burnable as at, StandardErc20 as au, Erc721 as av, Erc721LazyMintable as aw, Erc721ClaimableWithConditions as ax, Erc721WithQuantitySignatureMintable as ay, Erc721Supply as az, ClaimConditionInputArray as b, DEFAULT_RPC_URLS as b$, ClaimEligibility as b0, NotFoundError as b1, InvalidAddressError as b2, MissingRoleError as b3, AssetNotFoundError as b4, UploadError as b5, FileNameMissingError as b6, DuplicateFileNameError as b7, NotEnoughTokensError as b8, MissingOwnerRoleError as b9, fetchContractMetadataFromAddress as bA, fetchContractMetadata as bB, fetchSourceFilesFromMetadata as bC, fetchRawPredeployMetadata as bD, fetchPreDeployMetadata as bE, fetchExtendedReleaseMetadata as bF, detectFeatures as bG, isFeatureEnabled as bH, assertEnabled as bI, detectContractFeature as bJ, hasFunction as bK, toSemver as bL, isIncrementalVersion as bM, isDowngradeVersion as bN, OZ_DEFENDER_FORWARDER_ADDRESS as bO, CONTRACT_ADDRESSES as bP, getContractAddressByChainId as bQ, getContractPublisherAddress as bR, InterfaceId_IERC721 as bS, InterfaceId_IERC1155 as bT, NATIVE_TOKEN_ADDRESS as bU, NATIVE_TOKENS as bV, getNativeTokenByChainId as bW, EventType as bX, DEFAULT_IPFS_GATEWAY as bY, CHAIN_NAME_TO_ID as bZ, CHAIN_ID_TO_NAME as b_, QuantityAboveLimitError as ba, FetchError as bb, DuplicateLeafsError as bc, AuctionAlreadyStartedError as bd, FunctionDeprecatedError as be, ListingNotFoundError as bf, WrongListingTypeError as bg, RestrictedTransferError as bh, AdminRoleMissingError as bi, AuctionHasNotEndedError as bj, ExtensionNotImplementedError as bk, TransactionError as bl, convertToTWError as bm, includesErrorMessage as bn, createSnapshot as bo, hashLeafNode as bp, ALL_ROLES as bq, getRoleHash as br, extractConstructorParams as bs, extractFunctions as bt, extractConstructorParamsFromAbi as bu, extractFunctionParamsFromAbi as bv, extractFunctionsFromAbi as bw, extractEventsFromAbi as bx, resolveContractUriFromAddress as by, extractIPFSHashFromBytecode as bz, ClaimConditionOutputSchema as c, getProviderForNetwork as c0, getReadOnlyProvider as c1, EditionDropInitializer as c2, EditionInitializer as c3, MarketplaceInitializer as c4, MultiwrapInitializer as c5, NFTCollectionInitializer as c6, NFTDropInitializer as c7, PackInitializer as c8, SignatureDropInitializer as c9, FEATURE_NFT_REVEALABLE as cA, uploadOrExtractURIs as cB, AddressSchema as cC, AmountSchema as cD, BigNumberishSchema as cE, RawDateSchema as cF, PackContractSchema as cG, SplitsContractSchema as cH, fetchCurrencyValue as cI, DropErc20ContractSchema as cJ, TokenErc20ContractSchema as cK, VoteContractSchema as cL, SplitInitializer as ca, TokenDropInitializer as cb, TokenInitializer as cc, VoteInitializer as cd, PREBUILT_CONTRACTS_MAP as ce, CONTRACTS_MAP as cf, getContractTypeForRemoteName as cg, PAPER_API_URL as ch, parseChainIdToPaperChain as ci, fetchRegisteredCheckoutId as cj, createCheckoutLinkIntent as ck, PaperCheckout as cl, ContractWrapper as cm, DropErc1155ContractSchema as cn, TokenErc1155ContractSchema as co, MarketplaceContractSchema as cp, mapOffer as cq, isNativeToken as cr, MultiwrapContractSchema as cs, fetchCurrencyMetadata as ct, uploadOrExtractURI as cu, normalizePriceValue as cv, hasERC20Allowance as cw, isTokenApprovedForTransfer as cx, TokenErc721ContractSchema as cy, DropErc721ContractSchema as cz, ChainId as d, EditionMetadataWithOwnerOutputSchema as e, EditionMetadataInputSchema as f, EditionMetadataInputOrUriSchema as g, CommonContractOutputSchema as h, CommonRoyaltySchema as i, CommonPrimarySaleSchema as j, CommonPlatformFeeSchema as k, CommonTrustedForwarderSchema as l, CommonSymbolSchema as m, CurrencySchema as n, CurrencyValueSchema as o, Signature20PayloadInput as p, Signature20PayloadOutput as q, Signature721PayloadInput as r, Signature721PayloadOutput as s, Signature1155PayloadInput as t, Signature1155PayloadInputWithTokenId as u, Signature1155PayloadOutput as v, Signature721WithQuantityInput as w, Signature721WithQuantityOutput as x, MintRequest721 as y, MintRequest1155 as z };
