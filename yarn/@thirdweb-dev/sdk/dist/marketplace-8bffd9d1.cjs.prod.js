...
class Marketplace {
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(async e => {
      return await paperXyz.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
      ...
      case paperXyz.ListingType.Direct:
        {
          ...
          return await this.direct.makeOffer(listingId, quantity, paperXyz.isNativeToken(listing.currency) ? paperXyz.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
        }
